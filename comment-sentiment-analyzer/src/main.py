...
def test_model_with_examples():
    """Test the trained model with example comments"""
    
    model_path = Path(__file__).parent.parent / "models" / "best_integrated_model.pth"
    ...
    # Test comments
    test_comments = [
        # Quality + Sentiment examples
        "Amazing video quality! The editing is so smooth and professional",
        "Terrible video quality, couldn't even watch it",
        "Love the content but the camera work is shaky",
        "Great idea but the lighting is terrible",
        
        # Pure sentiment examples
        "This is so funny, made my day!",
        "This is boring, not for me",
    ...
    print("🧪 Testing Integrated Sentiment-Quality Model")
    print("=" * 70)
    
    for comment in test_comments:
        result = model.predict_comment(comment, tokenizer)
        
        print(f"\nComment: '{comment}'")
        print(f"Unified Score: {result['score']:.3f}")
        print(f"  └─ Sentiment: {result['sentiment']:.3f}")
        print(f"  └─ Quality Relevance: {result['quality_relevance']:.3f}")
        print(f"  └─ Quality Intensity: {result['quality_intensity']:.3f}")
...
def analyze_video_comments(comments_list):
    """Analyze a list of comments for a video"""
    
    model_path = Path(__file__).parent.parent / "models" / "best_sentiment_model.pth"  # ✅ Use the sentiment model we just trained
    
    if not model_path.exists():
        print("❌ Trained sentiment model not found!")
        return None
    
    # Load the pure sentiment model (not integrated)
    ...
    for comment in comments_list:
        ...
        try:
            ...
            with torch.no_grad():
                ...
                # Get sentiment prediction (-1 to 1)
                outputs = model(input_ids, attention_mask)
                sentiment_score = outputs['sentiment'].item()  # This is -1 to 1
    ...
    if not scores:
        return {
            'avg_score': 0.0,  # Neutral
            'comment_count': 0,
            'score_range': [0.0, 0.0]
        }
    
    return {
        'avg_score': float(np.mean(scores)),  # -1 to 1 range
